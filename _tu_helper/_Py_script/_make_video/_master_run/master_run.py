...
def to_toml_literal(value):
    ...
    if isinstance(value, (int, float)):
        # Для float лучше использовать repr, чтобы не потерять точность
        return repr(value)
    if isinstance(value, str):
        # Экранируем обратный слэш и кавычки
        val = value.replace('\\', '\\\\').replace('"', '\\"')
        return f'"{val}"'
    if isinstance(value, list):
        # Рекурсивно сериализуем элементы
        items = ", ".join(to_toml_literal(x) for x in value)
        return f"[{items}]"
    # Для нелинейных типов (dict, date, etc.) замену не делаем
...
def replace_key_in_toml_text(toml_text: str, key: str, new_value) -> tuple[str, bool]:
    """
    Заменяет значение для ключа (только если строка формата key = ... существует).
    Не трогаем закомментированные строки (# в начале).
    Возвращает (новый_текст, было_ли_заменено).
    """
...
def apply_params_to_file(config_path: Path, values: dict, only_keys: list[str] | None) -> dict:
    """
    Пытается заменить ключи в TOML-файле. Меняем только существующие строки вида "key = ...".
    Возвращает словарь статистики: {'changed': [...], 'missing': [...], 'skipped': [...]}.
    """
...
def main():
    ...
    for idx, step in enumerate(steps, start=1):
        ...
        # Обработка set_params (если требуется этим шагом)
        ...
        # Обработка run_script (если требуется этим шагом)
