...
"""
draw_graph.py — простая и максимально комментированная версия с фиксом дублей
и корректной миграцией формата стрелок к [[blocks.outs]].

Приоритет значений:
  Ручные (в [[blocks]]) > Авто (auto_*) > Defaults (run.toml)

Авто-параметры пересчитываются КАЖДЫЙ запуск, если нет ручных и freeze-флагов.
Направление раскладки по умолчанию: слева → направо (LR).
"""
...
# ---- TOML (stdlib, Python 3.11+)
try:
    import tomllib  # type: ignore
except Exception as e:
    print("ERROR: Нужен Python 3.11+ (есть tomllib).", e)
    sys.exit(1)

# ---- PNG экспорт (опционально)
_HAS_CAIROSVG = False
try:
...
class Defaults:
    ...
    # Нормированные размеры блока (0..1)
    ...
    style_label_offset: float = 0.5      # 0..1 (0.5 = по центру)
...
class PersistCfg:
    ...
    persist_auto_fields: List[str] = field(default_factory=lambda: ["pos","size","shape","fonts","style","text"])
...
class Block:
    ...
    text_align_title: Optional[str] = None
    text_align_props: Optional[str] = None
    text_valign_title: Optional[str] = None
    ...
    # Авто (пересчитываются при каждом запуске)
    ...
    # Локальные freeze (если когда-нибудь понадобятся)
    ...
    # Вычисляемые px (внутренняя геометрия)
    w_px: float = 0.0
    h_px: float = 0.0
    cx_px: float = 0.0
    cy_px: float = 0.0

    # Эффективные значения (ручные > авто > дефолт)
...
class Arrow:
    ...
    # Ручные стили
    style_curve: Optional[str] = None
    style_label_offset: Optional[float] = None
    style_connector_from: Optional[str] = None
    ...
    def eff(self, d: Defaults) -> Dict[str, object]:
        return {
            ...
            "label_offset": (
                self.style_label_offset if self.style_label_offset is not None
...
# ========= Парсинг файла графа + миграция в памяти =========
...
def parse_diagram(path: Path) -> Diagram:
    ...
    # 1) Вложенные outs в блоках (новый формат)
    for b in blocks_raw:
        frm = b.get("id")
        outs = b.get("outs")
        # вариант после миграции: список таблиц
        if isinstance(outs, list) and outs and isinstance(outs[0], dict):
            for o in outs:
                ...
                if not to:
                    continue
                arrows.append(Arrow(
                    from_id=frm, to_id=str(to), label=o.get("label"),
        ...
        # устаревший вариант: outs = ["B","C"] — поддержим на чтение
        if isinstance(outs, list) and outs and isinstance(outs[0], str):
            for to in outs:
                arrows.append(Arrow(from_id=frm, to_id=str(to)))

    # 2) Глобальные [[arrows]] — для обратной совместимости
    for a in data.get("arrows", []) or []:
        frm, to = a.get("from"), a.get("to")
        if not frm or not to:
            continue
        arrows.append(Arrow(
            from_id=str(frm), to_id=str(to), label=a.get("label"),
    ...
    # 3) Дедупликация стрелок
    uniq: Dict[Tuple, Arrow] = {}
    for a in arrows:
        key = (a.from_id, a.to_id, a.label, a.style_curve, a.style_connector_from, a.style_connector_to, a.style_label_offset)
        if key not in uniq:
            uniq[key] = a
...
# ========= Простая layered-раскладка =========

def layered_layout(di: Diagram, rc: RunConfig) -> None:
    """Простейшая укладка по слоям, задаёт auto_pos тем, у кого нет pos."""
...
# ========= Пиксели и разводка от наложений =========
...
def resolve_overlaps(di: Diagram, rc: RunConfig, W: int, H: int) -> None:
    ...
    # назад в auto_pos (если нет ручного pos)
    for b in di.blocks:
        if b.pos is None:
...
def render_svg(di: Diagram, d: Defaults, out_svg: Path) -> str:
    ...
    for b in di.blocks:
        ...
        if shape in ("rect","rounded","pill"):
            ...
            parts.append(f'<rect x="{x:.1f}" y="{y:.1f}" rx="{rx}" ry="{ry}" width="{w:.1f}" height="{h:.1f}" '
        ...
        parts.append(f'<text x="{tx:.1f}" y="{ty:.1f}" text-anchor="{tanchor}"{dom} '
                     f'font-family="{ffam}" font-size="{fsize_t}" fill="{text_col}" font-weight="{fw}">'
    ...
    for a in di.arrows:
        ...
        # подпись
        if a.label:
            t = float(st["label_offset"]); t = max(0.0, min(1.0, t))
            lx = p1[0] + (p2[0]-p1[0]) * t
...
# ========= Запись auto_* + миграция TOML на диск =========
...
AUTO_KEYS_OUT = ["auto_curve", "auto_label_offset", "auto_connector_from", "auto_connector_to"]

def _fmt(val) -> str:
    if isinstance(val, float):
        s = f"{val:.4f}"; return s.rstrip("0").rstrip(".") if "." in s else s
    if isinstance(val, bool):
        return "true" if val else "false"
    if isinstance(val, (list, tuple)) and len(val) == 2 and all(isinstance(x, float) for x in val):
        a, b = val; return f"[{_fmt(a)}, {_fmt(b)}]"
    return str(val)

def write_auto_and_migrate(path: Path, di: Diagram, rc: RunConfig) -> None:
    """
    Полная перезапись секций [[blocks]]:
      - удаляем старые auto_* и любые старые [[blocks.outs]]/outs=[...]
      - пишем новые auto_* (по включённым группам persist_auto_fields)
      - затем пишем уникальные [[blocks.outs]]
      - глобальные [[arrows]] вычищаем
      - секции [[blocks]] с повторным id пропускаем (сохраняем только первую)
    """
    text = read_text(path)
    ...
    def is_any_header(ln: str) -> bool:
        t = ln.strip()
        return t.startswith("[") and t.endswith("]")

    def is_block_header(ln: str) -> bool:
        return ln.strip().startswith("[[blocks]]")

    def is_block_outs_header(ln: str) -> bool:
        return ln.strip().startswith("[[blocks.outs]]")
    ...
    # дедуп стрелок (ещё раз)
    unique = {}
    for a in di.arrows:
        key = (a.from_id, a.to_id, a.label, a.style_curve, a.style_connector_from, a.style_connector_to, a.style_label_offset)
        if key in unique: continue
        unique[key] = a
    di.arrows = list(unique.values())
    ...
    while i < len(lines):
        ...
        # выкидываем глобальные [[arrows]] полностью
        ...
        if is_block_header(line):
            # собрать ВЕСЬ блок (включая ВСЕ [[blocks.outs]]) до след. [[blocks]] или иного заголовка
            j = i + 1
            sect = [line]
            while j < len(lines):
                nxt = lines[j]
                if is_block_header(nxt):
                    break
                if is_any_header(nxt) and not is_block_outs_header(nxt):
                    break
                sect.append(nxt); j += 1
            ...
            if m:
                ...
                # дубликат секции блока — полностью выкидываем
                if bid in seen_block_ids:
                    i = j
                    out.pop()  # удалить заголовок [[blocks]], который мы уже добавили
                    continue
                seen_block_ids.add(bid)

                blk = by_id.get(bid)
                if blk:
                    # 1) убрать старые auto_* в самой секции
                    sec_text = strip_auto_from_section(sec_text)

                    # 2) вычистить ВСЕ [[blocks.outs]] и простые "outs = [...]" из секции
                    sec_lines = sec_text.splitlines()
                    cleaned: List[str] = []
                    k = 0
                    while k < len(sec_lines):
                        st = sec_lines[k].strip()
                        if is_block_outs_header(sec_lines[k]):
                            k += 1
                            while k < len(sec_lines):
                                s2 = sec_lines[k].strip()
                                if is_any_header(s2):
                                    break
                                k += 1
                            continue
                        if st.startswith("outs") and "=" in st:
                            k += 1
                            continue
                        cleaned.append(sec_lines[k]); k += 1
                    sec_text = "\n".join(cleaned)

                    # 3) записать новые auto_* (по группам persist + freeze)
                    def append_block_auto(sec_text: str, b: Block) -> str:
                        # POS
                        if "pos" in rc.persist.persist_auto_fields and not (b.freeze_auto_pos or rc.persist.freeze_auto_pos):
                            if b.pos is None and b.auto_pos is not None:
                                x = clamp01(round_to_grid(b.auto_pos[0], rc.layout.grid_snap))
                                y = clamp01(round_to_grid(b.auto_pos[1], rc.layout.grid_snap))
                                sec_text += f'\nauto_pos = [{_fmt(x)}, {_fmt(y)}]'
                        # SIZE
                        if "size" in rc.persist.persist_auto_fields and not (b.freeze_auto_size or rc.persist.freeze_auto_size):
                            if b.width is None and b.auto_width is not None:
                                sec_text += f'\nauto_width = {_fmt(b.auto_width)}'
                            if b.header_height is None and b.auto_header_height is not None:
                                sec_text += f'\nauto_header_height = {_fmt(b.auto_header_height)}'
                            if b.prop_height is None and b.auto_prop_height is not None:
                                sec_text += f'\nauto_prop_height = {_fmt(b.auto_prop_height)}'
                        # SHAPE
                        if "shape" in rc.persist.persist_auto_fields and not (b.freeze_auto_shape or rc.persist.freeze_auto_shape):
                            if b.shape is None and b.auto_shape is not None:
                                sec_text += f'\nauto_shape = {b.auto_shape!r}'
                            if b.corner_radius is None and b.auto_corner_radius is not None:
                                sec_text += f'\nauto_corner_radius = {_fmt(b.auto_corner_radius)}'
                        # STYLE
                        if "style" in rc.persist.persist_auto_fields and not (b.freeze_auto_style or rc.persist.freeze_auto_style):
                            if b.stroke_width is None and b.auto_stroke_width is not None:
                                sec_text += f'\nauto_stroke_width = {_fmt(b.auto_stroke_width)}'
                            if b.fill is None and b.auto_fill is not None:
                                sec_text += f'\nauto_fill = {b.auto_fill!r}'
                            if b.stroke is None and b.auto_stroke is not None:
                                sec_text += f'\nauto_stroke = {b.auto_stroke!r}'
                        # FONTS
                        if "fonts" in rc.persist.persist_auto_fields and not (b.freeze_auto_fonts or rc.persist.freeze_auto_fonts):
                            if b.font_family is None and b.auto_font_family is not None:
                                sec_text += f'\nauto_font_family = {b.auto_font_family!r}'
                            if b.font_size_title is None and b.auto_font_size_title is not None:
                                sec_text += f'\nauto_font_size_title = {_fmt(b.auto_font_size_title)}'
                            if b.font_size_prop is None and b.auto_font_size_prop is not None:
                                sec_text += f'\nauto_font_size_prop = {_fmt(b.auto_font_size_prop)}'
                            if b.font_bold_title is None and b.auto_font_bold_title is not None:
                                sec_text += f'\nauto_font_bold_title = {_fmt(b.auto_font_bold_title)}'
                            if b.font_italic_arrow is None and b.auto_font_italic_arrow is not None:
                                sec_text += f'\nauto_font_italic_arrow = {_fmt(b.auto_font_italic_arrow)}'
                        # TEXT
                        if "text" in rc.persist.persist_auto_fields and not (b.freeze_auto_text or rc.persist.freeze_auto_text):
                            if b.text_align_title is None and b.auto_text_align_title is not None:
                                sec_text += f'\nauto_text_align_title = {b.auto_text_align_title!r}'
                            if b.text_align_props is None and b.auto_text_align_props is not None:
                                sec_text += f'\nauto_text_align_props = {b.auto_text_align_props!r}'
                            if b.text_valign_title is None and b.auto_text_valign_title is not None:
                                sec_text += f'\nauto_text_valign_title = {b.auto_text_valign_title!r}'
                            if b.text_valign_props is None and b.auto_text_valign_props is not None:
                                sec_text += f'\nauto_text_valign_props = {b.auto_text_valign_props!r}'
                            if b.text_padding_title_x is None and b.auto_text_padding_title_x is not None:
                                sec_text += f'\nauto_text_padding_title_x = {_fmt(b.auto_text_padding_title_x)}'
                            if b.text_padding_title_y is None and b.auto_text_padding_title_y is not None:
                                sec_text += f'\nauto_text_padding_title_y = {_fmt(b.auto_text_padding_title_y)}'
                            if b.text_padding_prop_x is None and b.auto_text_padding_prop_x is not None:
                                sec_text += f'\nauto_text_padding_prop_x = {_fmt(b.auto_text_padding_prop_x)}'
                            if b.text_padding_prop_y is None and b.auto_text_padding_prop_y is not None:
                                sec_text += f'\nauto_text_padding_prop_y = {_fmt(b.auto_text_padding_prop_y)}'
                        return sec_text

                    sec_text = append_block_auto(sec_text, blk)

                    # 4) добавить свежие [[blocks.outs]] из модели
                    if bid in outs_map:
...
def process_file(path: Path, rc: RunConfig) -> None:
    ...
    # Жёстко вырубаем auto_* перед построением (оставляем только ручные/дефолты)
    for b in di.blocks:
        b.auto_pos = b.auto_width = b.auto_header_height = b.auto_prop_height = None
        b.auto_shape = b.auto_corner_radius = None
        b.auto_stroke_width = b.auto_fill = b.auto_stroke = None
        b.auto_font_family = b.auto_font_size_title = b.auto_font_size_prop = None
        b.auto_font_bold_title = b.auto_font_italic_arrow = None
        b.auto_text_align_title = b.auto_text_align_props = None
        b.auto_text_valign_title = b.auto_text_valign_props = None
        b.auto_text_padding_title_x = b.auto_text_padding_title_y = None
        b.auto_text_padding_prop_x = b.auto_text_padding_prop_y = None

    # 1) Раскладка
    if rc.layout.mode == "layered":
        layered_layout(di, rc)

    # 2) Форс-пересчёт auto_* (если нет ручных и нет freeze)
    for b in di.blocks:

        # pos: если ручное есть — не держим auto
        if not (rc.persist.freeze_auto_pos or (b.freeze_auto_pos or False)):
            if b.pos is not None:
                b.auto_pos = None

        # size
        if not (rc.persist.freeze_auto_size or (b.freeze_auto_size or False)):
            b.auto_width = None if b.width is not None else rc.defaults.block_width
            b.auto_header_height = None if b.header_height is not None else rc.defaults.block_header_height
            b.auto_prop_height = None if b.prop_height is not None else rc.defaults.block_prop_height

        # shape/style
        if not (rc.persist.freeze_auto_shape or (b.freeze_auto_shape or False)):
            b.auto_shape = None if b.shape is not None else rc.defaults.block_shape
            b.auto_corner_radius = None if b.corner_radius is not None else rc.defaults.block_corner_radius
        if not (rc.persist.freeze_auto_style or (b.freeze_auto_style or False)):
            b.auto_stroke_width = None if b.stroke_width is not None else rc.defaults.block_stroke_width
            b.auto_fill = None if b.fill is not None else rc.defaults.block_fill
            b.auto_stroke = None if b.stroke is not None else rc.defaults.block_stroke

        # fonts
        if not (rc.persist.freeze_auto_fonts or (b.freeze_auto_fonts or False)):
            b.auto_font_family = None if b.font_family is not None else rc.defaults.font_family
            b.auto_font_size_title = None if b.font_size_title is not None else rc.defaults.font_size_title
            b.auto_font_size_prop = None if b.font_size_prop is not None else rc.defaults.font_size_prop
            b.auto_font_bold_title = None if b.font_bold_title is not None else rc.defaults.font_bold_title
            b.auto_font_italic_arrow = None if b.font_italic_arrow is not None else rc.defaults.font_italic_arrow

        # text
        if not (rc.persist.freeze_auto_text or (b.freeze_auto_text or False)):
            b.auto_text_align_title = None if b.text_align_title is not None else rc.defaults.text_align_title
            b.auto_text_align_props = None if b.text_align_props is not None else rc.defaults.text_align_props
            b.auto_text_valign_title = None if b.text_valign_title is not None else rc.defaults.text_valign_title
            b.auto_text_valign_props = None if b.text_valign_props is not None else rc.defaults.text_valign_props
            b.auto_text_padding_title_x = None if b.text_padding_title_x is not None else rc.defaults.text_padding_title_x
            b.auto_text_padding_title_y = None if b.text_padding_title_y is not None else rc.defaults.text_padding_title_y
            b.auto_text_padding_prop_x = None if b.text_padding_prop_x is not None else rc.defaults.text_padding_prop_x
            b.auto_text_padding_prop_y = None if b.text_padding_prop_y is not None else rc.defaults.text_padding_prop_y

    # 3) Пиксели и разводка
    ...
    # 5) Запись auto_* и миграция формата
...
def main() -> None:
    ...
    if not files:
        print("WARN: не найдено TOML-файлов. Проверь input_dirs в run.toml.")
        return
