...
"""
draw_graph.py — версия с принудительной очисткой auto_* перед построением
и последующей регенерацией по значениям из run.toml.

Приоритет: Ручные > Авто(из дефолтов после очистки) > Дефолты
"""
...
def parse_diagram(path: Path) -> Diagram:
    ...
    # глобальные [[arrows]] — обратная совместимость
...
# [AUTO-RESET] вспомогательная очистка auto_* в модели
def clear_auto(di: Diagram) -> None:
    for b in di.blocks:
        b.auto_pos = None
        b.auto_width = None
        b.auto_header_height = None
        b.auto_prop_height = None
        b.auto_shape = None
        b.auto_corner_radius = None
        b.auto_stroke_width = None
        b.auto_fill = None
        b.auto_stroke = None
        b.auto_font_family = None
        b.auto_font_size_title = None
        b.auto_font_size_prop = None
        b.auto_font_bold_title = None
        b.auto_font_italic_arrow = None
        b.auto_text_align_title = None
        b.auto_text_align_props = None
        b.auto_text_valign_title = None
        b.auto_text_valign_props = None
        b.auto_text_padding_title_x = None
        b.auto_text_padding_title_y = None
        b.auto_text_padding_prop_x = None
        b.auto_text_padding_prop_y = None
    for a in di.arrows:
        a.auto_curve = None
        a.auto_label_offset = None
        a.auto_connector_from = None
        a.auto_connector_to = None
...
def write_auto_and_migrate(path: Path, di: Diagram, rc: RunConfig) -> None:
    ...
    Полная перезапись секций [[blocks]]:
      ...
      - пишем новые auto_* (по persist_auto_fields)
    ...
    while i < len(lines):
        ...
        if is_block_header(line):
            # собрать ВЕСЬ блок, включая все [[blocks.outs]]
            j = i + 1
            sect = [line]
            while j < len(lines):
                nxt = lines[j]
            ...
            if m:
                ...
                blk = by_id.get(bid)
                if blk:
                    # 1) убрать старые auto_* в секции
                    sec_text = strip_auto_from_section(sec_text)

                    # 2) удалить все [[blocks.outs]] и "outs = [...]"
                    ...
                    # 3) записать новые auto_* (по persist + freeze)
...
def process_file(path: Path, rc: RunConfig) -> None:
    ...
    # [AUTO-RESET] 1) ЖЁСТКАЯ очистка всех авто-полей, считанных из файла
    clear_auto(di)

    # 2) Раскладка — выставит свежие auto_pos, если нужно
    if rc.layout.mode == "layered":
        layered_layout(di, rc)

    # 3) Регенерация auto_* из дефолтов (только где нет ручных и нет freeze)
    for b in di.blocks:
        # pos: если ручное есть — не ставим авто
    ...
    # 6) Запись auto_* и миграция формата
